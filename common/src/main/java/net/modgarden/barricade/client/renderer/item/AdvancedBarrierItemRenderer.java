...
public class AdvancedBarrierItemRenderer {
	...
	public static BakedModel createModel(AdvancedBarrierModelValues values) {
		...
		if (values.icon() != null) {
			Material material = new Material(InventoryMenu.BLOCK_ATLAS, values.icon());
			textureMap.put("barricade_layer" + i, Either.left(material));
			++i;
		}

		if (values.directions().blocksAll()) {
			textureMap.put("barricade_layer" + i, Either.left(new Material(InventoryMenu.BLOCK_ATLAS, BARRIER_TEXTURE)));
			++i;
		} else if (values.directions().doesNotBlock()) {
			textureMap.put("barricade_layer" + i, Either.left(new Material(InventoryMenu.BLOCK_ATLAS, NO_BARRIER_TEXTURE)));
			++i;
		} else {
			textureMap.put("barricade_layer" + i, Either.left(new Material(InventoryMenu.BLOCK_ATLAS, NO_BARRIER_TEXTURE)));
			++i;
			...
			for (Direction entry : values.directions().directions()) {
				textureMap.put("barricade_layer" + i, Either.left(new Material(InventoryMenu.BLOCK_ATLAS, DIRECTION_TO_MATERIAL_LOCATION.get(entry))));
				++i;
			}
		}

		BlockModel blockModel = new BlockModel(ResourceLocation.withDefaultNamespace("builtin/generated"), List.of(), textureMap, false, BlockModel.GuiLight.FRONT, Objects.requireNonNull(Minecraft.getInstance().getItemRenderer().getItemModelShaper().getItemModel(Items.BARRIER), "barrier item model must exist").getTransforms(), List.of());

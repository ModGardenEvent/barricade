...
public class AdvancedBarrierBlockUnbakedModel extends BlockModel {
	public static final Material BARRIER = new Material(InventoryMenu.BLOCK_ATLAS, Barricade.asResource("block/barrier"));
	...
	private static Map<String, Either<Material, String>> createTextureMap(@Nullable ResourceLocation icon) {
		Material innerMaterial = new Material(InventoryMenu.BLOCK_ATLAS, ResourceLocation.withDefaultNamespace("missingno"));
		if (icon != null)
			innerMaterial = new Material(InventoryMenu.BLOCK_ATLAS, icon);
	...
	@Override
	public @NotNull BakedModel bake(@NotNull ModelBaker modelBaker, @NotNull Function<Material, TextureAtlasSprite> textureGetter, @NotNull ModelState modelState) {
